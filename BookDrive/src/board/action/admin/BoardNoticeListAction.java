...
import board.svc.admin.BoardListService;
...
public class BoardNoticeListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String flag = "1"; //boardFlag(1 : 공지사항)
		...
		int listCount = boardListService.getListCount(kind, keyword, flag); //총 리스트 수를 받아옴 (총 글 갯수)
		boardList = boardListService.selectBoardList(nowPage, limit, kind, keyword, flag, os);
		...
		request.setAttribute("pagefile", "/board/admin_boardNoticeList.jsp?page="+nowPage+"&kind="+kind+"&keyword="+keyword+"&os="+os+"&limit="+limit);
