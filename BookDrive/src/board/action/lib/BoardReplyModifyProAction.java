...
import board.svc.lib.BoardModifyProService;
...
//		boardNum = boardNoticeWriteProService.selectBoardNotice(boardBean);
		...
		if (!isUpdateCheck) {
			...
			out.println("<script>");
			out.println("alert('답글 수정에 실패했습니다. 다시 확인해주세요.')");
...
//			forward.setPath("BoardModifyForm.boardA?boardnum=" + boardNum + "&page=" + page);
			forward.setPath("BoardQnADetail.boardL?boardnum=" + boardNum_p + "&page=" + page);
