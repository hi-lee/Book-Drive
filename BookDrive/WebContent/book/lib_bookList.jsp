	...
	<div id="divContents">
		...
		<div id="divLocation">
			<ul>
				...
				<li><a href="BookList.bookL">도서검색</a></li>
			</ul>
		</div>
		<!-- content -->
		<div id="divContent">
			<div class="searchBrief">
				...
				<div class="dataSearch">
					<form name="refineSearch" method="post" action="BookList.bookL">
						<input type="hidden" id="libcode" name="libcode" value="${sessionScope.code}">
						<fieldset>
							<select id="si1" name="flag" title="검색종류" class="searchOpt1 selectBox" style="display: none;">
								<option value="bookName" ${param.flag eq 'bookName' ? 'selected' : ''}>도서이름</option>
								<option value="bookNum" ${param.flag eq 'bookNum' ? 'selected' : ''}>도서번호</option>
								<option value="ISBN" ${param.flag eq 'ISBN' ? 'selected' : ''}>ISBN</option>
							</select>
							<select id="si2" name="bookstate" title="도서대여상태" class="searchOpt1 selectBox" style="display: none;">
				...
				<div class="briefContent">
					<div id="result" class="result">
						...
						<c:forEach var="booklist" items="${bookList}" varStatus="status">
							...
							<c:choose>
								<c:when test="${param.flag eq 'bookName'}">
									<c:set var="bookname" value="${fn:replace(booklist.bookName, param.keyword, replace_hilight)}"/>
								</c:when>
								<c:when test="${param.flag eq 'bookNum'}">
									<c:set var="booknum" value="${fn:replace(booklist.bookNum, param.keyword, replace_hilight)}"/>
								</c:when>
								<c:when test="${param.flag eq 'ISBN'}">
						...
						<div class="paging">
							<div class="paging">
								<c:choose>
									...
									<c:otherwise>
										<a href="BookList.bookL?page=1&libcode=${param.libcode}&keyword=${param.keyword}" class="page">
											<img src="bdstyle/admin/image/ko/solution/common/btn/firstPage.gif">
										</a>
										<a href="BookList.bookL?page=${pageinfo.nowPage-1}&libcode=${param.libcode}&keyword=${param.keyword}" class="page">
								...
								<span>
									<c:forEach begin="${pageinfo.startPage}" end="${pageinfo.endPage}" step="1" var="a">
										<c:choose>
											...
											<c:otherwise>
												<a href="BookList.bookL?page=${a}&libcode=${param.libcode}&keyword=${param.keyword}">${a}</a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</span>
								<c:choose>
									...
									<c:otherwise>
										...
										<a href="BookList.bookL?page=${pageinfo.maxPage}&libcode=${param.libcode}&keyword=${param.keyword}" class="page">
