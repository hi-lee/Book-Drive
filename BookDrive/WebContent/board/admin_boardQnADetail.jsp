	...
	<div id="divContents">
		...
		<div id="divLocation">
			<ul>
				...
				<li><a href="https://cham.kiu.ac.kr/bbs/content/2_5030#">QnA</a></li>
			</ul>
		</div>
		<div id="divContent">
			<div class="boardDetailW">
				...
				<div class="buttons">
					<c:choose>
						<c:when test="${!empty boardBean_c}">
							<a href="BoardReplyModifyForm.boardA?p_boardnum=${param.boardnum}&boardnum=${boardBean_c.boardNum}&page=${param.page}">답변수정</a>
						</c:when>
						<c:otherwise>
							<a href="BoardRepleWriteForm.boardA?boardnum=${param.boardnum}&page=${param.page}">답변하기</a>
						</c:otherwise>
					</c:choose>
					<a href="BoardModifyForm.boardA?boardnum=${param.boardnum}&page=${param.page}">질문글수정</a>
					<c:choose>
						<c:when test="${!empty boardBean_c}">
							<a href="javascript:deleteBoard(${param.boardnum}, ${param.page}, ${boardBean_p.boardFlag}, ${boardBean_c.boardNum});">질문글삭제</a>
						</c:when>
						<c:otherwise>
							<a href="javascript:deleteBoard(${param.boardnum}, ${param.page}, ${boardBean_p.boardFlag}, null);">질문글삭제</a>
						</c:otherwise>
					</c:choose>
