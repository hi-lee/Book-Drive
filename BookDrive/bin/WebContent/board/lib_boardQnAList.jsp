	...
	<div id="divContents">
		...
		<div id="divLocation">
			<ul>
				...
				<li><a href="javascript:void();">QnA 리스트</a></li>
			</ul>
		</div>
		<!-- sns share기능 사용여부 -->
		...
		<div id="divContent">
			...
			<div class="searchArea">
				...
				<form name="searchFrm" id="searchFrm" method="get" action="BoardList2.boardL">
								...
								<c:when test="${!empty boardList}">
									<c:forEach var="list" items="${boardList}">
										<tr>
											<td>${list.rowNum}</td>
											<td>
											<a href="BoardQnADetail.boardL?boardnum=${list.boardNum}&page=${pageinfo.nowPage}">
												${list.boardSubject}
											...
											<td class="footable-last-column">
												<c:choose>
													<c:when test="${list.boardReplyFlag eq '0'}">
														<a href="BoardRepleWriteForm.boardL?boardnum=${list.boardNum}&page=${pageinfo.nowPage}" class="btn bg_blue c_fff">답변하기</a>
													</c:when>
													<c:otherwise>
			...
			<div class="paging">
				<div class="paging">
					<c:choose>
						...
						<c:otherwise>
							<a href="BoardList2.boardL?libcode=${sessionScope.code}&page=1&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}" class="page">
								<img src="bdstyle/admin/image/ko/solution/common/btn/firstPage.gif">
							</a>
							<a href="BoardList2.boardL?libcode=${sessionScope.code}&page=${pageinfo.nowPage-1}&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}" class="page">
					...
					<span>
						<c:forEach begin="${pageinfo.startPage}" end="${pageinfo.endPage}" step="1" var="a">
							<c:choose>
								...
								<c:otherwise>
									<a href="BoardList2.boardL?libcode=${sessionScope.code}&page=${a}&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}">${a}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
					</span>
					<c:choose>
						...
						<c:otherwise>
							...
							<a href="BoardList2.boardL?libcode=${sessionScope.code}&page=${pageinfo.maxPage}&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}" class="page">
