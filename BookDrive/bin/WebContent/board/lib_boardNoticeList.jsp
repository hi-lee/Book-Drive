	...
	<div id="divContents">
		...
		<div id="divLocation">
			<ul>
				...
				<li><a href="javascript:void();">공지사항 리스트</a></li>
			</ul>
		</div>
								...
								<c:when test="${!empty boardList}">
									<c:forEach var="list" items="${boardList}">
										<tr>
											...
											<a href="BoardNoticeDetail.boardL?boardnum=${list.boardNum}&page=${pageinfo.nowPage}">
												${list.boardSubject}
											</a>
											</td>
			...
			<div class="paging">
				<div class="paging">
					<c:choose>
						...
						<c:otherwise>
							<a href="BoardList1.boardL?page=1&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}" class="page">
								<img src="bdstyle/admin/image/ko/solution/common/btn/firstPage.gif">
							</a>
							<a href="BoardList1.boardL?page=${pageinfo.nowPage-1}&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}" class="page">
					...
					<span>
						<c:forEach begin="${pageinfo.startPage}" end="${pageinfo.endPage}" step="1" var="a">
							<c:choose>
								...
								<c:otherwise>
									<a href="BoardList1.boardL?page=${a}&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}">${a}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
					</span>
					<c:choose>
						...
						<c:otherwise>
							...
							<a href="BoardList1.boardL?page=${pageinfo.maxPage}&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}" class="page">
