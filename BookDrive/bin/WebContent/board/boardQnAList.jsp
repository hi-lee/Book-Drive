...
<link rel="stylesheet" href="bdstyle/style/ko/standard/list.css">
	...
	<div id="divContents">
		...
		<div id="divContent">
			...
			<div class="searchArea">
				...
				<form name="searchFrm" id="searchFrm" method="get" action="qnaBoard.bo">
					<fieldset>
						<legend>검색</legend>
						<span class="bunch">
			...
			<div class="listInfo">
				<div class="listInfo1">
					<p class="totalCnt">
						총 <span class="totalcount">${pageinfo.listCount}</span> 건
					</p>
					<p class="pageNum">
				...
				<div class="listInfo2">
					<div class="fileSend">
						<ul>
							<li>
							 	<a class="btnType1" href="wishBoardWriteForm.bo?flag=3" title="글작성">
							 		글작성
							 	</a>
							</li>
						</ul>
						...
						<tbody>
							<c:choose>
								<c:when test="${!empty boardList}">
									<c:set var="emptychk" value="false"/>
									<c:forEach var="list" items="${boardList}" varStatus="status">
										<c:if test="${list.memIndex eq sessionScope.userIndex}">
											<c:set var="emptychk" value="true"/>
											<tr>
												<td class="num footable-first-column">${status.index +((pageinfo.page-1)*5)+1 }</td>
												<td class="title expand">
												<a href="qnaboardDetail.bo?boardNum=${list.boardNum}&page=${pageinfo.page}">
													${list.boardSubject}
												...
												<td class="footable-last-column">
													<c:choose>
														<c:when test="${list.boardReplyFlag eq '0'}">
															<a class="btn bg_blue c_fff">미답변</a>
														</c:when>
														<c:otherwise>
			...
			<div class="paging">
				<div class="paging">
					<c:choose>
						...
						<c:otherwise>
							<a href="freeBoard.bo?page=1&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}" class="page">
								<img src="bdstyle/admin/image/ko/solution/common/btn/firstPage.gif">
							</a>
							<a href="freeBoard.bo?page=${pageinfo.page-1}&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}" class="page">
					...
					<span>
						<c:forEach begin="${pageinfo.startPage}" end="${pageinfo.endPage}" step="1" var="a">
							<c:choose>
								...
								<c:otherwise>
									<a href="freeBoard.bo?page=${a}&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}">${a}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
					</span>
					<c:choose>
						...
						<c:otherwise>
							...
							<a href="freeBoard.bo?page=${pageinfo.maxPage}&kind=${param.kind}&keyword=${param.keyword}&os=${param.os}&countPerPage=${param.countPerPage}" class="page">
